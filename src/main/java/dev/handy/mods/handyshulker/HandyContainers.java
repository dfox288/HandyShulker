 ...
 *
 * Each method consults {@link HandyShulkerConfig} to respect the global
...
public final class HandyContainers {
	...
	public static boolean isSupported(ItemStack stack) {
		...
		if (EnderChestHelper.isEnderChest(stack) && HandyShulkerConfig.get().enableEnderChestSupport) return true;
		return false;
	}
	...
	public static List<ItemStack> getContents(ItemStack stack, Player player) {
		...
		if (EnderChestHelper.isEnderChest(stack) && HandyShulkerConfig.get().enableEnderChestSupport && player != null) {
	...
	public static int tryInsert(ItemStack container, Player player, ItemStack toInsert) {
		...
		if (EnderChestHelper.isEnderChest(container) && HandyShulkerConfig.get().enableEnderChestSupport && player != null) {
	...
	private static void syncIfServer(Player player) {
		if (player instanceof ServerPlayer sp) {
			HandyShulker.syncEnderChest(sp);
		}
	}
}
